    ...
    $(document).on("submit",
        ...
        function (e) {
            ...
            for (var i = 0; i < form[0].length; i++) {
                if (form[0][i].getAttribute('type') == 'submit') {
                    submitBtn = form[0][i];
                    activeLoader(submitBtn)
                }
            }
            if (method === "get") {
                ...
                $.ajax({
                    ...
                    error: function (data) {
                        showAlert("خطایی رخ داده است. لطفا با مدیر سیستم تماس بگیرید.", "error")
                        disabledLoader(submitBtn)
                    }
                });
            }
            return false;
...
function CallBackHandler(data, action, form) {
    switch (action) {
        ...
        case "Refresh":
            ...
            if (data.isSuccess) {
                activeLoader(submitBtn)
                showAlert("عملیات موفقیت آمیز بوده است.", "success")
...
function showAlert(messageText, state) {
...
function activeLoader(btn) {
    debugger
    btn.disabled = true;
    spanBtn = `<span style="display:none">${btn.textContent}</span>`;
    finalBtn = `${spanBtn}<div id="form_loader"></div>`;
    btn.innerHTML = finalBtn;
}
function disabledLoader(btn) {
    btn.disabled = false;
    btnText = btn.firstChild.textContent
